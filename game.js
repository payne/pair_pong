...
Game.prototype.loop = function ( context ) {
	...
	$( 'canvas' ).on( 'keydown keyup', function ( keyEvent ) {
		...
		//??? toAscii('A')==65
		if ( keyEvent.type === 'keydown' && 74 == keyEvent.which ) {
			self.down = true;
			self.up = false;
		}
		if ( keyEvent.type === 'keydown' && 75 == keyEvent.which ) {
			self.down = false;
			self.up = true;
		}
...
Game.prototype.update = function ( context, down, up ) {
	...
	this.entities.forEach( function ( entity ) {
		if ( entity.update ) {
			entity.update( context, down, up, paddle ); // only call if there is an update method
		}
	} );
}
...
$( document ).ready( function () {
	...
	ball.paddle = paddle;
	ball.update = function ( context, up, down ) {
		if ( this.intersect( this.paddle ) ) {
			this.xVelocity *= -1;
			this.yVelocity += 10;
		}
		//   else if ( ball.intersect( computerPaddle ) ) {
		//  this.xVelocity *= -1;
		//  this.yVelocity += 10;
		// }
		Entity.prototype.update.apply( this, arguments );
